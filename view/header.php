...
$adrs = $_SESSION['pageAdrs'];
$AreaListQ = "select * from area where m_id = $mid ";
$AreaListR = $conn->query($AreaListQ);
// print_r($adrs);
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="image-box" style="background-image: url('../admin/includes/images/bg-01.jpg'); background-size: cover;">
            ...
            <ul class="navbar-nav mx-auto justify-content-center">
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                        <?php
        while ($row = mysqli_fetch_assoc($AreaListR)) { ?>
                        <form action="<?php echo $adrs ?>" method="post">
                            <input type="hidden" name="reqar" value=<?php echo $row['a_id']; ?>>
                            <input class='dropdown-item' type="submit" value=<?php echo $row['a_name']; ?>>
                        </form>
                        <?php }
        ?>
                    </div>
            ...
            echo $_SESSION['f_name']." ".$_SESSION['l_name'];
                ...
                <div class="dropdown-menu bg-dark " aria-labelledby="navbarDropdownMenuLink">
                    ...
                    <Button class="btn btn-md btn-dark"
                        ...
                        Profile</Button>
                    <Button class="btn btn-md btn-dark" onclick="logout('<?php echo $_SESSION['user_id'] ?>');">Log
                        Out</Button>
                </div>
            </span>
        </div>
...
if (isset($_POST['updateCPass']))
    ...
    if (isset($_POST['updateFname']) || isset($_POST['updateLname']) ) 
    {
      ...
      if ($conn->query($updt)) {
        ...
        // echo "<script>
        //   location.replace('http://localhost/fyp/login/index.php')
        // </script>";
        // exit();
        header("Location: index.php");
    }
